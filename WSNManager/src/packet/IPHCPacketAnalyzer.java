...
//source https://github.com/contiki-os/contiki/blob/master/tools/cooja/java/org/contikios/cooja/plugins/analyzers/IPv6PacketAnalyzer.java
public class IPHCPacketAnalyzer extends PacketAnalyzer {
	...
	public final static int SICSLOWPAN_UDP_8_BIT_PORT_MAX = 0xF0FF; /*
																	 * F000 +
																	 * 255
																	 */
	...
	public boolean matchPacket(Packet packet) {
		return packet.level == NETWORK_LEVEL
				&& (packet.get(0) & 0xe0) == IPHC_DISPATCH;
	}
	...
	public int analyzePacket(Packet packet, StringBuilder brief,
		...
		verbose.append("TF = ").append(tf).append(", NH = ")
				.append(nhc ? "compressed" : "inline").append(", HLIM = ")
				.append(hlim == 0 ? "inline" : hlim).append(", CID = ")
				.append(cid).append(", SAC = ")
				.append(sac == 0 ? "stateless" : "stateful").append(", SAM = ")
				.append(sam).append(", MCast = ").append(m).append(", DAC = ")
				.append(dac == 0 ? "stateless" : "stateful").append(", DAM = ")
				.append(dam);
		if (cid == 1) {
		...
		try {
			...
			if ((packet.get(0) & SICSLOWPAN_IPHC_FL_C) == 0) {
				...
				if ((packet.get(0) & SICSLOWPAN_IPHC_TC_C) == 0) {
					...
					trafficClass = ((tmp >> 2) & 0x3f) | (tmp << 6)
							& (0x80 + 0x40);
					/*
					 * ECN rolled down two steps + lowest DSCP bits at top two
					 * bits
					 */
				} else {
				...
				if ((packet.get(1) & SICSLOWPAN_IPHC_DAC) != 0) {
					...
					case SICSLOWPAN_IPHC_DAM_11: /* 0 bits */
						...
						System.arraycopy(packet.llreceiver, 0, destAddress,
								16 - packet.llreceiver.length,
								packet.llreceiver.length);
						break;
					}
				} else {
					/*
					 * not context based => link local M = 0, DAC = 0 - same as
					 * SAC
					 */
					switch (packet.get(1) & SICSLOWPAN_IPHC_DAM_11) {
					case SICSLOWPAN_IPHC_DAM_00: /* 128 bits */
			...
			if (nhc) {
				...
				if ((packet.get(hc06_ptr) & SICSLOWPAN_NDC_UDP_MASK) == SICSLOWPAN_NHC_UDP_ID) {
					...
					case (byte) SICSLOWPAN_NHC_UDP_01:
						...
						destPort = SICSLOWPAN_UDP_8_BIT_PORT_MIN
								+ (packet.get(hc06_ptr + 3) & 0xFF);
						hc06_ptr += 6;
						break;
					case (byte) SICSLOWPAN_NHC_UDP_10:
						/* 1 byte for NHC, 3 byte for ports, 2 bytes chksum */
						srcPort = SICSLOWPAN_UDP_8_BIT_PORT_MIN
								+ (packet.get(hc06_ptr + 1) & 0xFF);
						destPort = packet.getInt(hc06_ptr + 2, 2);
						hc06_ptr += 6;
						break;
					case (byte) SICSLOWPAN_NHC_UDP_11:
						/* 1 byte for NHC, 1 byte for ports, 2 bytes chksum */
						srcPort = SICSLOWPAN_UDP_4_BIT_PORT_MIN
								+ (packet.get(hc06_ptr + 1) >> 4);
						destPort = SICSLOWPAN_UDP_4_BIT_PORT_MIN
								+ (packet.get(hc06_ptr + 1) & 0x0F);
						hc06_ptr += 4;
						break;
					}
				}
				...
				if (proto == EXT_HDR_HOP_BY_HOP) {
					...
					// header length is length specified in field, rounded up to
					// 64 bit
		...
		// IPv6 Information
		brief.append("|IPv6");
		verbose.append("<br/><b>IPv6</b>").append(" TC = ")
				.append(trafficClass).append(", FL = ").append(flowLabel)
				.append("<br>");
		verbose.append("From ");
		...
		if (proto == PROTO_UDP || proto == PROTO_ICMP || proto == PROTO_TCP) {
